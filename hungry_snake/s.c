...
#define SHAPE_FOOD       '*'
#define SHAPE_SNAKE      '#'
#define GAMEWIN_YLEN     25
#define GAMEWIN_XLEN     56
#define LOGWIN_YLEN      7
#define LOGWIN_XLEN      55
...
int main()
{
    while (true)
    {
        ...
        RUNLOG("  'q' or 'Q' to quit.");
        RUNLOG("  'p' to pause the game.");
        RUNLOG("  'w/s/a/d' or 'W/S/A/D' to move the snake.");
        //RUNLOG("Info:");
        
        WINDOW *gwin = newgamew(); /* 创建游戏窗口，具体由自定义函数newgamew实现 */
        mvwprintw(gwin, GAMEWIN_YLEN/2, GAMEWIN_XLEN/2, " Level: 1");
        mvwprintw(gwin, GAMEWIN_YLEN/2+1, GAMEWIN_XLEN/2, " Level: 2");
        mvwprintw(gwin, GAMEWIN_YLEN/2+2, GAMEWIN_XLEN/2, " Level: 3");
        wrefresh(gwin);
        char ch = getch();
        if(ch == '1') g_level = 1;
        else if(ch == '2') g_level = 2;
        else if(ch == '3') g_level = 3;
        else g_level = 1;
        //mvwprintw(gwin, GAMEWIN_YLEN-1, 2, " Level: %d ", g_level);
        mvwprintw(gwin, GAMEWIN_YLEN/2, GAMEWIN_XLEN/2, "         ");
        mvwprintw(gwin, GAMEWIN_YLEN/2+1, GAMEWIN_XLEN/2, "         ");
        mvwprintw(gwin, GAMEWIN_YLEN/2+2, GAMEWIN_XLEN/2, "         ");
        wrefresh(gwin);
...
int refreshgamew(WINDOW *win, struct TSnake *psnake)
{
    ...
    timeout.tv_sec = 0;
    timeout.tv_usec= (5 - (int)(g_level)) * 50*1000;
    ...
    case 1:
        ...
        mvwprintw(win, GAMEWIN_YLEN-1, 2, " amount: %d ", amount);
        mvwprintw(win, GAMEWIN_YLEN-1, 30, " Speed: %d ", (int)(g_level));
        wrefresh(win);
        //RUNLOG("Level UP!");
...
WINDOW* newgamew()
{
    ...
    box(win, 0, 0); // 绘制窗口的边框, /* 0, 0 是字符默认的行列起始位置*/
    mvwprintw(win, 0, 2, " GAME "); //将光标移动到窗口的0行2列的位置，并打印信息
...
struct TSnake* initsnake()
{
    ...
    newsnakenode( &psnake->head, 4, 50 )->front = psnake->head;
...
WINDOW* newlogw()
{
    WINDOW *win = newwin(LOGWIN_YLEN, LOGWIN_XLEN, 1, GAMEWIN_XLEN + 5);
    box(win, 0, 0);
    mvwprintw(win, 0, 2, " manual ");
    wrefresh(win);
